...
from .config import mccth_config
...
class MCCTH(BaseCF):
    ...
    def init(self, first_frame, bbox):
        ...
        for i in range(self.expert_num):
            ...
            self.experts[i].hf_num=np.conj(self.yf)[:,:,None]*xtf/(self.cf_response_size[0]*self.cf_response_size[1])
    ...
    def update(self, current_frame, vis=False):
        ...
        if self.frame_idx>=self.period-1:
            ...
            ave_score=np.sum(np.array(self.mean_score)[self.period:self.frame_idx+1]*
                             np.array(self.psr_score[self.period:self.frame_idx+1]))/(self.frame_idx+1-self.period)
