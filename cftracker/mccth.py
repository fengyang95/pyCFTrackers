...
class MCCTH(BaseCF):
    ...
    def init(self, first_frame, bbox):
        ...
        if self.scale_adaptation is True:
            ...
            self.min_scale_factor = self.scale_step ** (
                int(np.ceil(np.log(max(5 / self.bg_area[0], 5 / self.bg_area[1])) /
    ...
    def update(self, current_frame, vis=False):
        ...
        likelihood_map = self.get_colour_map(im_patch_cf, self.bg_hist, self.fg_hist, self.bin_mapping)
        likelihood_map=cv2.resize(likelihood_map,self.cf_response_size,cv2.INTER_LINEAR)
        likelihood_map[np.isnan(likelihood_map)] = 0.
        likelihood_map=(likelihood_map+np.min(likelihood_map))/(np.min(likelihood_map)+np.max(likelihood_map))
        if np.sum(likelihood_map)*self.cf_response_size[0]*self.cf_response_size[1]<0.01:
            likelihood_map=1
        likelihood_map=np.clip(likelihood_map,a_min=0.1,a_max=None)

        xt = self.get_feature_map(im_patch_cf, self.hog_cell_size)
        xt = self._window[:, :, None] * xt*likelihood_map[:,:,None]
        xt_cn, xt_hog1, xt_hog2 = self.split_features(xt)
        self.experts[0].xt = xt_cn
        self.experts[1].xt = xt_hog1
        self.experts[2].xt = xt_hog2
        ...
        for i in range(self.expert_num):
            ...
            response_cf = self.mex_resize(crop_filter_response(response_cf, response_sz), self.norm_delta_area)
            response_cf[np.isnan(response_cf)] = 0.
            self.experts[i].response = response_cf
        ...
        if vis is True:
            ...
            self.crop_size=(int(self.bg_area[0]),int(self.bg_area[1]))
        ...
        for i in range(self.expert_num):
            ...
            self.experts[i].hf_den = (1 - self.learning_rate_cf) * self.experts[
                i].hf_den + self.learning_rate_cf * hf_den
            self.experts[i].hf_num = (1 - self.learning_rate_cf) * self.experts[
                i].hf_num + self.learning_rate_cf * hf_num
        if self.learning_rate_pwp != 0:
            im_patch_color = self.get_sub_window(current_frame, self._center, self.norm_bg_area,(self.bg_area[0]*(1-self.inner_padding),
                                                                                                 self.bg_area[1]*(1-self.inner_padding)))
            self.bg_hist, self.fg_hist = self.update_hist_model(self.new_pwp_model,
                                                                im_patch_color, self.bg_area, self.fg_area, self.target_sz,
