...
class Staple(BaseCF):
    ...
    def init(self,first_frame,bbox):
        ...
        self.bg_hist,self.fg_hist=self.update_hist_model(self.new_pwp_model,patch_padded,self.bg_area,self.fg_area,
    ...
    def update(self,current_frame,vis=False):
        ...
        pwp_search_area=(round(self.norm_pwp_search_area[0]/self.area_resize_factor),round(self.norm_pwp_search_area[1]/self.area_resize_factor))
        ...
        if self.scale_adaptation:
            im_patch_scale=self.get_scale_subwindow(current_frame,self._center,self.base_target_sz,self.scale_factor*self.scale_factors,
    ...
    def compute_histogram(self,patch,mask,n_bins):
        ...
        histogram=cv2.calcHist([patch],[0,1,2],mask,[n_bins,n_bins,n_bins],[0,256,0,256,0,256])/np.count_nonzero(mask)
        return histogram
