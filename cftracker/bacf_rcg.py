...
from .config.bacf_rcg_config import BACFRCGConfig
from .cf_utils import mex_resize,resp_newton,resize_dft2
from .scale_estimator import LPScaleEstimator


class BACFRCG(BaseCF):
    def __init__(self, config=BACFRCGConfig()):
        super(BACFRCG).__init__()
        ...
        self.admm_tao=config.admm_tao
        self.admm_mu=config.admm_mu
    ...
    def init(self,first_frame,bbox):
        ...
        self.gaussian_win=self.gaussian_window(self.feature_map_sz,(self.base_target_sz[0]//self.cell_size,
    ...
    def ADMM(self,xf):
        ...
        #zf = np.sum(xf * g_f, axis=2) - self.yf
        zf=np.zeros((xf.shape[0],xf.shape[1]))
        r_f=np.zeros_like(zf)
        mu=2.5
        tao = 2.5
        beta = 1.05
        mu_max = 100
        ...
        xs, ys, _ = self.get_subwindow_no_window(zf, (int(self.feature_map_sz[0] / 2), int(self.feature_map_sz[1] / 2)),
                                                 self.small_filter_sz)
        P = np.zeros((self.feature_map_sz[1], self.feature_map_sz[0]), dtype=np.complex64)
        P[ys, xs] = 1
        ...
        while i <= self.admm_iterations:
            ...
            tmp0 = 1 / mu * (tao * self.yf[:, :, None] * xf + tao * xf * zf[:, :, None] - xf * r_f[:, :,
                                                                                               None] - l_f + mu * h_f)
            tmp1 = tao * self.yf * S_xx + tao * zf * S_xx - r_f * S_xx - S_lx + mu * S_hx
            g_f = tmp0 - xf * tmp1[:, :, None] / (mu * B[:, :, None])

            # solve for h
            h = (1 / (mu + self.admm_lambda)) * ifft2(mu * g_f + l_f)
            h_f = (fft2(h * P[:, :, None]))

            # solve for z
            z=1/(self.gaussian_win*self.gaussian_win+tao)*np.real(ifft2(tao*(np.sum(xf*g_f,axis=2)-self.yf)+r_f))
            zf=fft2(z)

            l_f = l_f + (mu * (g_f - h_f))
            r_f=r_f+tao*(np.sum(xf*g_f,axis=2)-self.yf-zf)

            tao=min(beta*tao,tao_max)
    ...
    def gaussian_window(self,window_sz,target_sz,delta):
        W,H=window_sz
        w,h=target_sz
        window=np.zeros((H,W))
        x_range=np.arange(-(W/2-w/2),W/2-w/2).astype(np.int)+W//2
        y_range=np.arange(-(H/2-h/2),H/2-h/2).astype(np.int)+H//2
        xs,ys=np.meshgrid(x_range,y_range)
        window[ys,xs]=np.exp(-0.5*(((xs-W//2)/(w*delta))**2+((ys-H//2)/(h*delta))**2))
        #window[ys,xs]=1.
        #window[:,:]=1
        window=window.T
        #t=window/np.max(window)*255
        #cv2.imshow('win',t.astype(np.uint8))
        #cv2.waitKey(0)
        return window
