...
from cftracker.config.bacf_config import BACFConfig

from got10k.trackers import Tracker

class BACF(Tracker): #更改 base -> Tracker
    def __init__(self, config=BACFConfig()): 
        super(BACF, self).__init__(name='BACF', is_deterministic=True) #更改
    ...
    def init(self,first_frame,bbox):
        first_frame = np.array(first_frame)[:,:,::-1] #更改
    ...
    def update(self,current_frame,vis=False):
        current_frame = np.array(current_frame)[:,:,::-1] #更改
        x=None
